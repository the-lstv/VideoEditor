 ...
 * @experimental
 */
export default class PianoRoll extends LS.Timeline {
    constructor(options = {}) {
        super({
...
void main() {
    float d = roundedBoxSDF((v_uv - 0.5) * size, size * 0.5, 5.0);
    ...
    setNotes(notes) {
        console.log("Setting notes in piano roll:", notes);
        this.reset(notes, false);
    }
    ...
    getLabel(item) {
        return [item.label || this.getNoteName(item.row), 68];
    }
}